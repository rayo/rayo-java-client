...
public class NettyServerHandler extends SimpleChannelHandler {
	...
	public void sendRayoOffer() {
		
		offerId = UUID.randomUUID().toString();
		callId = UUID.randomUUID().toString();
		String offer = "<presence type=\"set\" id=\"%s\" from=\"%s@localhost\" to=\"userc@localhost/voxeo\"> " + 
		   "<offer xmlns=\"urn:xmpp:rayo:1\" to=\"sip:userc@localhost:5060\" from=\"sip:test@someip.com:6089\">" +
	...
	private void processIQMessage(String message) {
		...
		if (iq.getChildName().equals("say") || 
			...
			iq.getChildName().equals("ask") ||
			iq.getChildName().equals("dial") ||
			iq.getChildName().equals("join")) {
