...
package com.voxeo.rayo.client.auth.sasl;
...
import com.voxeo.servlet.xmpp.rayo.stanza.sasl.AuthMechanism;
import com.voxeo.servlet.xmpp.rayo.stanza.sasl.Response;
...
public abstract class SASLMechanism implements CallbackHandler {
    ...
    public SASLMechanism(XmppConnection connection) {
     ...
     */
    public void challengeReceived(String challenge) throws IOException, XmppException {
    	
        byte response[];
        if(challenge != null) {
            response = sc.evaluateChallenge(org.apache.commons.codec.binary.Base64.decodeBase64(challenge));
        } else {
            response = sc.evaluateChallenge(new byte[0]);
        }

        XmppObject responseStanza;
        if (response == null) {
            responseStanza = new Response();
        }
        else {
        	String text = org.apache.commons.codec.binary.Base64.encodeBase64String(response);
            responseStanza = new Response(text);
        }
     ...
     */
    protected abstract AuthMechanism.Type getName();
